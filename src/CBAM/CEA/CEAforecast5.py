...
class CEAPricePredictor:
    def __init__(self, data):
        ...
        self.ml_models = {}  # 存储机器学习模型
        self.ml_predictions = {}  # 存储机器学习预测结果
    ...
    def create_ml_features(self, df):
        ...
        # 基础价格特征
        data['price_lag1'] = data['close(RMB/ton)'].shift(1)
        data['price_lag3'] = data['close(RMB/ton)'].shift(3)
        data['price_lag5'] = data['close(RMB/ton)'].shift(5)
        
        # 移动平均特征
        data['ma5'] = data['close(RMB/ton)'].rolling(window=5).mean()
        data['ma10'] = data['close(RMB/ton)'].rolling(window=10).mean()
        
        # 价格变化特征
        data['price_change_1d'] = data['close(RMB/ton)'].pct_change(1)
        data['price_change_3d'] = data['close(RMB/ton)'].pct_change(3)
        ...
        # 波动率特征
        data['volatility_5d'] = data['close(RMB/ton)'].rolling(window=5).std()
        
        # 目标变量（未来1天的价格）
        data['target'] = data['close(RMB/ton)'].shift(-1)
    ...
    def train_ml_models(self):
        """训练机器学习模型"""
        # 创建特征
        feature_data = self.create_ml_features(self.data)
    
        # 修复：增加数据量检查并提高要求
        if len(feature_data) < 20:  # 从10提高到20，确保有足够数据
            print(f"数据量不足（当前{len(feature_data)}条），无法训练机器学习模型")
            print("建议收集更多数据后重试")
            return {}
    
    # 特征和标签
        feature_columns = ['price_lag1', 'price_lag3', 'price_lag5', 'ma5', 'ma10', 
                        'price_change_1d', 'price_change_3d', 'day_of_week', 
                         'day_of_month', 'month', 'volatility_5d']
    
        X = feature_data[feature_columns]
        y = feature_data['target']
    
    # 修复：确保测试集索引在有效范围内
        test_size = min(0.2, 0.5 * len(feature_data) / len(feature_data))  # 动态调整测试集大小
        if len(feature_data) < 30:
            test_size = 0.1  # 数据量少时使用更小的测试集
    ...
    # 修复：安全地获取测试集索引
        ...
        valid_test_indices = [idx for idx in test_indices if idx in feature_data.index]
    
        if len(valid_test_indices) == 0:
            print("错误：无法找到有效的测试数据索引")
            return {}
    ...
    # 修复：安全地获取测试日期
        try:
            test_dates = feature_data.loc[valid_test_indices, 'date'].values
            test_actual = feature_data.loc[valid_test_indices, 'target'].values
        except Exception as e:
            print(f"获取测试日期时出错: {e}")
        # 备用方案：使用索引创建日期
            test_dates = [feature_data['date'].iloc[0] + timedelta(days=i) for i in range(len(valid_test_indices))]
    ...
    # 计算评估指标
        lr_mae = mean_absolute_error(test_actual, lr_pred)
        lr_rmse = np.sqrt(mean_squared_error(test_actual, lr_pred))
        lr_r2 = r2_score(test_actual, lr_pred)
    
        rf_mae = mean_absolute_error(test_actual, rf_pred)
        rf_rmse = np.sqrt(mean_squared_error(test_actual, rf_pred))
        rf_r2 = r2_score(test_actual, rf_pred)
    ...
    def plot_ml_fitting(self):
        ...
        # 修复：检查数据是否有效
        lr_data = self.ml_predictions['LinearRegression']
        rf_data = self.ml_predictions['RandomForest']
    
        if len(lr_data['test_dates']) == 0 or len(rf_data['test_dates']) == 0:
            print("错误：测试数据为空，无法绘制图表")
            return
    
        fig, axes = plt.subplots(2, 1, figsize=(14, 10))
    
        # 第一个子图：线性回归拟合情况
        axes[0].plot(lr_data['test_dates'], lr_data['test_actual'], 
                'b-', label='实际价格', linewidth=2, alpha=0.8)
        axes[0].plot(lr_data['test_dates'], lr_data['pred'], 
                'r--', label='线性回归预测', linewidth=2, alpha=0.8)
        axes[0].set_title('线性回归模型拟合情况')
        axes[0].set_ylabel('价格 (元/吨)')
        axes[0].legend()
        axes[0].grid(True, alpha=0.3)
    
        # 添加误差统计
        mae = mean_absolute_error(lr_data['test_actual'], lr_data['pred'])
        rmse = np.sqrt(mean_squared_error(lr_data['test_actual'], lr_data['pred']))
        r2 = r2_score(lr_data['test_actual'], lr_data['pred'])
        axes[0].text(0.02, 0.98, f'MAE: {mae:.2f}\nRMSE: {rmse:.2f}\nR²: {r2:.3f}', 
                transform=axes[0].transAxes, verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    
    # 第二个子图：随机森林拟合情况
        axes[1].plot(rf_data['test_dates'], rf_data['test_actual'], 
                'b-', label='实际价格', linewidth=2, alpha=0.8)
        axes[1].plot(rf_data['test_dates'], rf_data['pred'], 
                'g--', label='随机森林预测', linewidth=2, alpha=0.8)
        axes[1].set_title('随机森林模型拟合情况')
    ...
    # 添加误差统计
        mae = mean_absolute_error(rf_data['test_actual'], rf_data['pred'])
        rmse = np.sqrt(mean_squared_error(rf_data['test_actual'], rf_data['pred']))
        r2 = r2_score(rf_data['test_actual'], rf_data['pred'])
    ...
    def plot_residuals(self):
        ...
        # 线性回归残差
        lr_data = self.ml_predictions['LinearRegression']
        lr_residuals = lr_data['test_actual'] - lr_data['pred']
        
        axes[0].scatter(lr_data['pred'], lr_residuals, alpha=0.6)
        ...
        # 随机森林残差
        rf_data = self.ml_predictions['RandomForest']
        rf_residuals = rf_data['test_actual'] - rf_data['pred']
        
        axes[1].scatter(rf_data['pred'], rf_residuals, alpha=0.6, color='green')
...
def main(number=30):
    ...
    # 1. 加载数据
    print("正在加载数据...")
    conn = sqlite3.connect("F:/project_of_datadesign/CBAM/datas_for_CBAM/cbam_database.db")
