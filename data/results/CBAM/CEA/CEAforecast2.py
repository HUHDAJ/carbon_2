...
def plot_comparison(historical_data, predictions_dict, title="CEA价格预测对比"):
    """
    绘制不同预测方法的对比图
    """
    # 修复：创建3x2的子图网格，总共6个子图位置
    ...
    # 隐藏多余的子图（如果有6个子图位置，但只有5个方法）
    if len(axes) > len(predictions_dict):
...
def print_results_table(results_dict, last_price):
    """打印结果表格"""
    print("=" * 80)
    print(f"{'预测方法':<15} {'日期':<12} {'价格':<10} {'累计变化':<12} {'日变化':<10}")
    print("-" * 80)
    
    methods = list(results_dict.keys())
    days = len(results_dict[methods[0]])
    
    for day in range(days):
        print(f"\n第{day+1}天预测结果:")
        for method in methods:
            result = results_dict[method][day]
            cumulative_sign = "+" if result['cumulative_change'] >= 0 else ""
            daily_sign = "+" if result['daily_change'] >= 0 else ""
            
            print(f"  {method:<12} {result['date']:<12} {result['price']:<10.2f} "
...
def main():
    ...
    # 3. 使用不同方法进行预测
    print("\n正在进行价格预测...")
    methods = ['original', 'v1', 'v2', 'v3', 'combined']
    results_dict = {}
    
    for method in methods:
        print(f"  - 使用 {method} 方法预测...")
        try:
            results = predictor.get_predictions(steps=300, method=method)
            results_dict[method] = results
        except Exception as e:
            print(f"    方法 {method} 预测失败: {e}")
    
    if not results_dict:
        print("所有预测方法都失败了，请检查数据格式。")
        return None, None
    
    # 4. 打印结果
    print_results_table(results_dict, predictor.last_price)
    
    # 5. 绘制对比图
    print("\n生成预测对比图...")
    try:
        plot_comparison(data, results_dict, "CEA价格预测对比")
    except Exception as e:
        print(f"绘图失败: {e}")
        # 尝试简化绘图
        print("尝试简化绘图...")
        plot_simple_comparison(data, results_dict)
    ...
    for method, results in results_dict.items():
        ...
        if method == 'original':
            ...
            score = 10 - abs(avg_change) - volatility * 2
    ...
    # 找出最佳预测方法
    best_method = max(trend_analysis.items(), key=lambda x: x[1]['score'])[0]
    best_results = results_dict[best_method]
    
    print(f"推荐预测方法: {best_method}")
    print(f"评估得分: {trend_analysis[best_method]['score']:.1f}/10")
    print(f"平均累计变化: {trend_analysis[best_method]['avg_change']:.2f}%")
    print(f"预测波动性: {trend_analysis[best_method]['volatility']:.2f}%")
    print()
    ...
    for result in best_results:
        ...
        print(f"  {result['day']}: {result['price']:.2f} "
              ...
              f"日变化{change_sign if result['daily_change'] >= 0 else ''}{result['daily_change']:.2f}%)")
...
if __name__ == "__main__":
    ...
    3. 获取预测: results = predictor.get_predictions(steps=5, method='combined')
