...
db_path = './datas_for_CBAM/cbam_database.db'
...
electric_carbon_area1 = pd.read_sql_query(f'SELECT * FROM electric_carbon_area1',conn)#全国区域电力平均排放因子（华东，华北...）
emit_of_iron_steel = pd.read_sql_query(f'SELECT * FROM emit_of_iron_steel',conn)#钢铁行业（以螺纹钢为例）排放因子
electric_carbon_area2 = pd.read_sql_query(f'SELECT * FROM electric_carbon_area2',conn)#各省份电力平均排放因子
...
#         return area1[area_level]*use_MWh
def electric_carbon_area(use_MWh,month1,day1):
    electric_carbon_daily['日期'] = pd.to_datetime(electric_carbon_daily['日期'])
    elec1 = electric_carbon_daily[(electric_carbon_daily['日期'].dt.month == month1) & (electric_carbon_daily['日期'].dt.day == day1)]
